...
#' 
#' 
#' Generates smoothed partial effect plots for continuous variables. The solid
#' black line represents the estimated partial effect; dashed red lines show an
#' approximate plus-minus standard error band. These standard errors are
#' intended as heuristic guides and should be interpreted cautiously.
#' 
...
#' effects are expected to be nonlinear. Use \code{parametric = TRUE} if the
#' underlying effect is believed to follow a global polynomial form.
#' 
#' For binary variables, partial effects are shown as boxplots, with whiskers
#' reflecting variability analogous to standard error.
#' 
#' The causal estimator, when requested, displays the baseline-subtracted
#' parametric local effect.
#' 
#' Conditioning is supported via the \code{subset} option. When supplied as a
#' factor (with length equal to the original data), the plot is stratified by
#' its levels. Alternatively, \code{subset} can be a logical or integer vector
#' indicating the cases to include in the analysis.
#' 
#' @param x \code{partialpro} object returned from a previous call to
#' \code{partialpro}.
#' @param xvar.names Names (or integer indices) of the x-variables to plot.
#' Defaults to all variables.
#' @param nvar Number of variables to plot. Defaults to all.
#' @param parametric Logical. Set to \code{TRUE} only if the partial effect is
#' believed to follow a polynomial form.
#' @param se Display standard errors?
#' @param causal Display causal estimator?
#' @param subset Optional conditioning factor. Not applicable if
#' \code{parametric = TRUE}. May also be a logical or integer vector to subset
#' the analysis.
#' @param plot.it If \code{FALSE}, no plot is produced; instead, partial effect
#' values are returned.
#' @param ... Additional arguments passed to \code{plot}.
#' @author
#' 
#' Min Lu and Hemant Ishwaran
#' @seealso \command{\link{partialpro}}
#' @references
#' 
